...
import com.mvavrill.logicGamesSolver.model.games.sudoku.grid.DeterministicGrid;
import com.mvavrill.logicGamesSolver.model.games.sudoku.grid.SudokuGrid;
import com.mvavrill.logicGamesSolver.model.games.sudoku.grid.WeightedGrid;
...
public class ImageProcessing {
    ...
    public static SudokuGrid weightedGridFromScratch(final double[][] image, final Context context) {
        Log.d("Mat","imageProcessing");
        boolean[][] img = new ImageProcessing(image).gaussianFilter(5,1.5).edgeFilter().binarizeBool(0.85);
        Log.d("Mat","binarized");
        ImageProcessing.removeLonely(img);
        Log.d("Mat","remove Lonely");
        Vertex[][] vertices = new HoughTransform(img).getSudokuEdges();
        Log.d("Mat","hough");
        boolean[][][][] imageDigits = new DigitImageExtraction(img, vertices).extract();
        Log.d("Mat","extracetd");
        MnistDetection mnist;
        try {
            mnist = new MnistDetection(new Scanner(context.getAssets().open("mnist/mnist_test_images.dat")));
        }
        catch (Exception e) {
            return new DeterministicGrid();
        }
        Log.d("Mat","mnistLoaded");
        DigitProbabilities[][] digits = new DigitProbabilities[9][9];
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                ImageProcessing.removeLonely(imageDigits[i][j]);
                digits[i][j] = mnist.digitsProbabilities(imageDigits[i][j]);
            }
        }
        Log.d("Mat","done");
        return new WeightedGrid(digits);
    }
  ...
  /*public static double[][] toGrayScale(final BufferedImage image) {
    ...
    public ImageProcessing edgeFilter() {
        return convolutionFilter(new double[][]{new double[]{1, 1, 1}, new double[]{1, -8, 1}, new double[]{1, 1, 1}});
    }
